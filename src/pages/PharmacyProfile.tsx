...
const PharmacyProfile = () => {
  ...
  const pharmacy = {
    ...
    medicines: [
      ...
      { name: 'Aspirin 75mg', price: 15, brand: 'Ecosprin', inStock: true, quantity: '14 tablets' },
      { name: 'Omeprazole 20mg', price: 65, brand: 'Omez', inStock: true, quantity: '10 capsules' },
      { name: 'Ciprofloxacin 500mg', price: 85, brand: 'Cipro', inStock: false, quantity: '10 tablets' },
      { name: 'Losartan 50mg', price: 95, brand: 'Cozaar', inStock: true, quantity: '15 tablets' }
    ],
  ...
  const addToCart = (medicine) => {
    ...
    const existingItem = selectedMedicines.find(item => item.name === medicine.name);
    if (existingItem) {
      ...
      setSelectedMedicines(prev => [...prev, { ...medicine, cartQuantity: 1 }]);
    }
  };

  const removeFromCart = (medicineName) => {
    setSelectedMedicines(prev => prev.filter(item => item.name !== medicineName));
  };
                  ...
                  <div key={index} className="bg-white p-4 rounded-lg shadow-md">
                    <div className="flex justify-between items-start mb-2">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-600">{medicine.brand} • {medicine.quantity}</p>
                        <p className="text-green-600 font-semibold">₹{medicine.price} each</p>
                      </div>
                      <button
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        <button
                          onClick={() => updateCartQuantity(medicine.name, medicine.cartQuantity - 1)}
                          className="w-8 h-8 bg-gray-200 rounded-full flex items-center justify-center hover:bg-gray-300"
                        >
                          -
                        </button>
                        <span className="font-semibold">{medicine.cartQuantity}</span>
                        <button
                          onClick={() => updateCartQuantity(medicine.name, medicine.cartQuantity + 1)}
                          className="w-8 h-8 bg-gray-200 rounded-full flex items-center justify-center hover:bg-gray-300"
                        >
                      ...
                      <p className="font-semibold text-gray-800">₹{medicine.price * medicine.cartQuantity}</p>
                    </div>
                  </div>
                ))}
              </div>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="p-4 space-y-6">
          ...
          <div className="p-6">
              ...
              <div className="flex-1 mt-4">
                ...
                <div className="flex items-center space-x-4 mt-2">
                  <div className="flex items-center space-x-1">
                    ...
                    <span className="text-sm text-gray-600">{pharmacy.rating} ({pharmacy.reviews} reviews)</span>
                  </div>
            ...
            <div className="mt-4 space-y-2">
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm text-gray-600">{pharmacy.openHours} • {pharmacy.distance} km away</span>
              </div>
              {pharmacy.delivery && (
                <div className="flex items-center space-x-2">
                  <Truck className="w-4 h-4 text-green-500" />
                  <span className="text-sm text-green-600">Home delivery available</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Search Medicines</h3>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-semibold text-gray-800">Available Medicines</h3>
            {selectedMedicines.length > 0 && (
              <button
          ...
          <div className="space-y-3">
            {filteredMedicines.map((medicine, index) => (
              <div key={index} className={`p-4 border rounded-lg ${
                medicine.inStock ? 'border-gray-200' : 'border-gray-300 bg-gray-50'
              }`}>
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600">{medicine.brand} • {medicine.quantity}</p>
                    <p className="text-green-600 font-semibold mt-1">₹{medicine.price}</p>
        ...
        <div className="bg-blue-50 p-4 rounded-lg">
          <p className="text-sm text-blue-800">
            <strong>Note:</strong> Medicine availability is updated in real-time. Call the pharmacy to confirm stock before visiting.
          </p>
        </div>
      </div>
    </div>
